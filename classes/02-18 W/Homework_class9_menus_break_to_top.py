...
State is what the program remembers as it is break_to_top, a transition is what causes the program to move from one state/level to another.
When the user picks a valid number option it enters the next while loop and if its none, the break exits the loop and returns to the parent loop.
An invariant is something always true no matter what the user does. 
For example, break_to_top is only ever set to True at a leaf, and only ever reset to False at the top of the outermost loop — never anywhere in between.
    b) Does menu control come from logic or program structure
Menu control comes from program structure, with logic only stepping in for the return-to-top escape.
